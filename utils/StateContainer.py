...
class StatePoint(PropertyDict):
    ...
    def __init__(self, position=None, **kwargs):
        ...
        self.position = position  # Simpan posisi    
    ...
    def __str__(self):
        return f"StatePoint({self.__dict__}, Position: {self.position})"
...
class StateContainer(object):
    ...
    def __getitem__(self, index):
        if self._list_like(index):
            ...
            if len_var == 0:
                ...
                if index[1] == 'Position':  # Tambahkan akses posisi
                    return self._points[index[0]].position
    ...
    def __setitem__(self, index, value):
        if self._list_like(index):
            ...
            if len_var == 0:
                ...
                if index[1] == 'Position':  # Cek apakah yang diset adalah posisi
                    if index[0] not in self._points:
                        self._points[index[0]] = StatePoint()
                    self._points[index[0]].position = value
                else:
                    if index[0] not in self._points:
                        self._points[index[0]] = StatePoint()
    ...
    def __str__(self):
        headers = ["state", "Position", "T (°C)", "p (bar)", "d (kg/m³)", "h (kJ/kg)", "s (kJ/kg·K)"]
        table_data = []

        for i in self._points:
            position = self._points[i].position if self._points[i].position else "-"
            
            # Konversi satuan untuk tampilan yang lebih umum
            T = self.units[CoolProp.iT].from_SI(self._points[i][CoolProp.iT])  # Kelvin ke Celsius
            P = self.units[CoolProp.iP].from_SI(self._points[i][CoolProp.iP])  # Pascal ke Bar
            #D = self.units[CoolProp.iDmass].from_SI(self._points[i][CoolProp.iDmass])  # Densitas tetap dalam kg/m³
            H = self.units[CoolProp.iHmass].from_SI(self._points[i][CoolProp.iHmass])   # J/kg ke kJ/kg
            S = self.units[CoolProp.iSmass].from_SI(self._points[i][CoolProp.iSmass])  # J/kg·K ke kJ/kg·K

            # Tambahkan data ke dalam tabel
            table_data.append([i, position, f"{T:.3f}", f"{P:.3f}",  f"{H:.3f}", f"{S:.3f}"])

        # Kembalikan tabel dalam format yang rapi
